  ...
  class SecurityDevicesController < ApplicationController
    ...
    private
      ...
      def security_device_params
        params.fetch(:security_device_security_device, {}).permit(:id, :apiotics_instance, :sound_sensor_attributes => ["data", "port_number", "id"], :motion_sensor_attributes => ["motion", "port_number", "id"], :vibration_sensor_attributes => ["threshold_exceeded", "port_number", "id"])
      end
  end
end